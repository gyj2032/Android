...
import com.allen.view.adapter.NewSmsAdapter;
...
public class NewSMSActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		addButton.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				if (null == inputNum || "".equals(inputNum.getText().toString())) {
				} else {
					String phoneNum = inputNum.getText().toString();
					if (isNum(inputNum.getText().toString().trim())) {
		...
		sendButton.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				if (null == selectContactList || selectContactList.size() < 1) {
					Toast.makeText(NewSMSActivity.this, "请输入发送目标",
		...
		if (null != getIntent().getStringExtra("list")) {
			...
			for (ContactBean cb : selectContactList) {
				...
				final View child = numLayout.getChildAt(numLayout.getChildCount() - 1);
				autoHeight(child);
			}
		}
	...
	private void initMyGroupView() {
		numLayout = (LinearLayout) findViewById(R.id.l1);
		...
		inputNum.setOnFocusChangeListener(new OnFocusChangeListener() {
			public void onFocusChange(View v, boolean hasFocus) {
				if (!hasFocus) {
					if (isNum(inputNum.getText().toString().trim())) {
						createView1(inputNum.getText().toString().trim(), inputNum
	...
	private void autoHeight(final View child) {
		if (child != null) {
			new Handler() {
				...
				public void run() {
					if (child.getBottom() > numLayout.getBottom()
							|| numLayout.getBottom() - child.getBottom() >= child
									.getHeight()) {
						LayoutParams l = numLayout.getLayoutParams();
						l.height = child.getBottom();
						numLayout.setLayoutParams(l);
					}
				}
			}, 500);
		}
	...
	private void createView1(String text, String number) {
		...
		if (inputNum.getText().toString().equals(" ")
			...
			t.setTag(number);
			numLayout.addView(t, numLayout.getChildCount() - 1);
	...
	private class MyListener implements OnClickListener {
		public void onClick(View v) {
			...
			autoHeight(numLayout.getChildAt(numLayout.getChildCount() - 1));
		}
	}
	...
	private void setAdapter(List<ContactBean> list) {
		...
		queryListView.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (b) {
					inputNum.setText(cb.getDisplayName());
					createView1(inputNum.getText().toString().trim(),
							cb.getPhoneNum());
					inputNum.setText("");
				} else {
					queryListView.setVisibility(View.INVISIBLE);

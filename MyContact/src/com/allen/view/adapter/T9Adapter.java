...
public class T9Adapter extends BaseAdapter implements Filterable {
	...
	public View getView(int position, View convertView, ViewGroup parent) {   
		...
		if (convertView == null) {
			...
			addViewListener(holder.call_btn, position);
		}else{
...
//			holder.pinyin.setVisibility(View.VISIBLE);
	...
	private void addViewListener(View view, final int position){
		view.setOnClickListener(new OnClickListener(){
			public void onClick(View view) {
				PhoneUtil.showOptionDialog(context, list.get(position).getDisplayName(), list.get(position).getPhoneNum());
			}
		});
	}

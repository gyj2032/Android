...
import com.allen.view.other.SystemScreenInfo;
import com.allen.view.ui.AnimationTabHost;
...
public class HomeTabHostAcitivity extends TabActivity {
	...
	private RecordService phoneService;
	
	private ImageView keypadSwithView;
	private int keypadImageId;
	...
	private int getImageId(int index, boolean isSelect){
		...
		switch (index) {
		case 0: 
			result = isSelect ? R.drawable.tab_dial_ic_close : R.drawable.tab_dial_normal;
			HomeDialActivity.getHomeDialActivity().dialPadShow(true);
			break;
		case 1:
...
//		setIndicator("拨号", 0, new Intent(this, HomeDialActivity.class), R.drawable.tab_dial_selected);
		setIndicator("拨号", 0, new Intent(this, HomeDialActivity.class), R.drawable.tab_dial_ic_close);
...
//		onPageSelected(1);
	...
	private void setIndicator(String ss, int tabId, Intent intent, int image_id) {
		...
		if (tabId == 0) {
			keypadSwithView = imageView;
			keypadImageId = image_id;
			localView.setOnTouchListener(new View.OnTouchListener() {
				@Override
				public boolean onTouch(View v, MotionEvent event) {
					if (MotionEvent.ACTION_UP == event.getAction()) {
						if (HomeDialActivity.getHomeDialActivity().dialPadShow()) {
							keypadImageId = R.drawable.tab_dial_ic_close;
						} else {
							keypadImageId = R.drawable.tab_dial_ic_open;
						}
						keypadSwithView.setImageResource(keypadImageId);
					}
					return false;
				}
			});
